...
def optimal_power_flow(*args):
    ...
    Aeq = sparse(ones(ng))
    beq = [Pd]
    # Aineq = sparse(vstack([Distribution_factor * Cg,-Distribution_factor * Cg]).toarray())
    # bineq = [branch[:,RATE_A] + Distribution_factor * Cd *bus[:,PD]] +[-branch[:,RATE_A] + Distribution_factor * Cd *bus[:,PD]]
    Aineq = Distribution_factor * Cg
    Aineq = vstack([Aineq, -Aineq])
    bineq = branch[:, RATE_A] + Distribution_factor * Cd * bus[:, PD]
    bineq_attach =  branch[:, RATE_A] - Distribution_factor * Cd * bus[:, PD]

    bineq = concatenate((bineq, bineq_attach))
    c = gencost[:,5]
    Q = diag(gencost[:, 4])
